...
function zonas($nom, $lineas, $links)
{
  ...
  foreach ($lineas as $linum => $linea) {
    if ($linum == 3) { // SOLO MOSTRAMOS SOFTWARE PUBLICO. QUITAR PARA VER TODO.
      ...
      // LINKS DE LA LINEA
      if ($haylink == 1) {
        $mobile .= '<div class="mapa-caja links-pais" id="links-' . $linum . '-' . $id . '">';
        //$mobile.= '<h2>'.$nom.'</h2><br><h4>'.$linea.'</h4>';
        foreach ($links[$linum] as $link => $titulo) {
          $mobile .= '<div class="map-data"><a target="_blank" href="' . $link . '">' . $titulo . '</a></div>';
        }
        $mobile .= '</div>';
      }
    }
  ...
  <div class="datit">' . $nom . '</div>
  <div class="datex">SOFTWARE PUBLICO</div>
  <div class="datmi">
    <ul>
      ';
  if (!empty($links)) {
    foreach ($links[3] as $link => $titulo) {
      $dev .= '<li><a target="_blank" href="' . $link . '">' . $titulo . '</a>';
    }
  }
  $dev .= '
    </ul>
...
foreach ($paises as $indice => $nompais) {
  $empty = array();
  $pais[$indice] = zonas($paises[$indice], $lineas, $empty);
}
